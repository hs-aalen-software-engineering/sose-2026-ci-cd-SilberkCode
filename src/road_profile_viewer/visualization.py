...
def create_dash_app() -> Dash:
    ...
    def update_graph(angle: float | None) -> tuple[go.Figure, str]:  # pyright: ignore[reportUnusedFunction]
        ...
        if x_intersect is not None and y_intersect is not None and distance is not None:
            # Ray goes from camera to intersection point
            x_ray = np.array([camera_x, x_intersect])
            y_ray = np.array([camera_y, y_intersect])
        else:
            # No intersection - show a short ray using the shared geometry helper.
            x_ray, y_ray = calculate_ray_line(angle, camera_x=camera_x, camera_y=camera_y, x_max=80)
